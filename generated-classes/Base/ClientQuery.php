 ...
 * @psalm-method ObjectCollection&\Traversable<ChildClient> find(ConnectionInterface $con = null) Return ChildClient objects based on current ModelCriteria
 * @method     ChildClient[]|ObjectCollection findById(int $id) Return ChildClient objects filtered by the id column
 * @psalm-method ObjectCollection&\Traversable<ChildClient> findById(int $id) Return ChildClient objects filtered by the id column
 * @method     ChildClient[]|ObjectCollection findByName(string $name) Return ChildClient objects filtered by the name column
 * @psalm-method ObjectCollection&\Traversable<ChildClient> findByName(string $name) Return ChildClient objects filtered by the name column
 * @method     ChildClient[]|ObjectCollection findByEmail(string $email) Return ChildClient objects filtered by the email column
 * @psalm-method ObjectCollection&\Traversable<ChildClient> findByEmail(string $email) Return ChildClient objects filtered by the email column
...
abstract class ClientQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'mydb', $modelName = '\\Client', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, email FROM client WHERE id = :p0';
        try {
